...
function validate( options ) {
	...
	if ( !isObject( options ) ) {
		return new TypeError( 'invalid input argument. Options must be an object.' );
	}
...
function encoding( value ) {
	if ( !isString( value ) && value !== null ) {
		return new TypeError( 'invalid option. Encoding option must be a string primitive or null. Option: `' + value + '`.' );
	}
	return null;
...
function highWaterMark( value ) {
	if ( !isNonNegative( value ) ) {
		return new TypeError( 'invalid option. High watermark option must be a nonnegative number. Option: `' + value + '`.' );
	}
	return null;
...
function objectMode( value ) {
	if ( !isBoolean( value ) ) {
		return new TypeError( 'invalid option. objectMode option must be a boolean primitive. Option: `' + value + '`.' );
	}
	return null;
